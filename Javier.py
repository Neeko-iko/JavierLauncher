...
class MainJavier(QtWidgets.QWidget):  # whoops sorry for the bad code down below!
    def __init__(self):  # my bad guys i promise my code is ususally better than this
        ...
        if os.name == "nt":
            self.ui.Tabs.setStyleSheet("{color : #DDDDDD;}")
    ...
    # Nontabbed buttosn/other code
        self.ui.logClearButton.clicked.connect(
            lambda: self.ui.miniSole.setPlainText(""))
    ...
    # Launcher Tab Code
        ...
        self.ui.serverRefreshButton.clicked.connect(
            lambda: self.refreshingServers())
        self.ui.searchBar.returnPressed.connect(
            lambda: self.refreshingServers())
    ...
    def refreshSettings(self):
        if self.ui.defaultCheck.isChecked():
            ...
            self.ui.jraEntry.setText(jdb.readServerValue(
                self.selectedServer, "LaunchFlags"))
            self.ui.sJavaOver.setText(jdb.readServerValue(
                self.selectedServer, "JavaFilePath"))
            self.ui.jarFileEntry.setText(
                jdb.readServerValue(self.selectedServer, "JARName"))
    ...
    def saveSettings(self):
        if self.ui.defaultCheck.isChecked():
            ...
            if jdb.readServer(name) == False:
                # it wasn't how could i forget these lines!!!????
                jdb.addServer(name)
            jdb.updateServerValue(name, "JavaFilePath",
                                  self.ui.sJavaOver.text())
    ...
    def addDir(self):
        direc = str(QtWidgets.QFileDialog.getExistingDirectory(
            self, "Select New Directory"))
        if direc != "":
    ...
    # answer: yes! very necessary! it's useful to read up on what you're using before just slamming into it at full force.
    ...
    def Startup(self):
        ...
        if name == None:  # i'll figure out a better way of doing this later
            self.printl(
                "You need to select a server! You can't just start nothing!")
            return
        self.saveSettings()
        ...
        if not os.path.isfile(pathers+"/eula.txt"):
            self.printl(
                "By starting this server using Javier you accept Minecraft's EULA defined at \nhttps://www.minecraft.net/en-us/eula\n")
    ...
    def refreshingServers(self, subs=True):
        ...
        self.SButtonFrames = QtWidgets.QGridLayout(
        ...
        for i in range(0, len(directories)):
            ...
            for i in range(0, len(servers)):
                ...
                self.serverButton.clicked.connect(
                    lambda _=False, e=server, d=dire: self.setServer(e, d))
                self.favorButton.clicked.connect(
                    lambda _=False, e=server, d=self.favorButton: self.favoritism(e, d))
                ...
                if len(self.normal["buttons"]) + len(self.favorites["buttons"]) > 10:
                    self.ui.scrollAreaWidgetContents.setFixedHeight(
                        self.ui.scrollAreaWidgetContents.height() + 45)
            listsize = len(self.favorites["buttons"]) + \
                len(self.normal["buttons"])
            height = int(500/(listsize+.1) - 10) if listsize < 10 else 45

            for i in range(0, len(self.favorites["buttons"])):
                self.favorites["buttons"][i].setFixedSize(500, height)
                self.SButtonFrames.addWidget(
                    self.favorites["buttons"][i], i+1, 1, 1, 1)
                self.SButtonFrames.addWidget(
                    self.favorites["checks"][i], i+1, 0, 1, 1)
            # this disgusts you as much as it does me
            for i in range(0, len(self.normal["buttons"])):
                self.normal["buttons"][i].setFixedSize(500, height)
                self.SButtonFrames.addWidget(
                    self.normal["buttons"][i], i+len(self.favorites["buttons"])+1, 1, 1, 1)
                self.SButtonFrames.addWidget(
                    self.normal["checks"][i], i+len(self.favorites["buttons"])+1, 0, 1, 1)
    ...
    def refreshingDirs(self, subs=True):
        ...
        for i in range(0, len(directs)):
            ...
            self.deldirbutton.clicked.connect(
                lambda _=False, d=dire: self.delDirs(d))
            ...
            self.directButton.clicked.connect(
                lambda _=False, e=dire: self.openDir(e))
            self.dirbuttlist.append(self.directButton)
            if len(self.dirbuttlist) > 10:
                self.ui.dirScrollerWidget.setFixedHeight(
                    self.ui.dirScrollerWidget.height() + 45)
    ...
    # i absolutely love reusing HUGE chunks of code 3 times because im incompetent!!! it *will* happen again.
    def refreshingThemes(self, subs=True):
        if subs:
        ...
        # this makes it ignore the current theme (it doesn't and i don't know how.)
        self.deftheme.setStyleSheet('')
        ...
        for theme in themelist:
            ...
            self.themebutton.clicked.connect(
                lambda _=False, d=theme: self.updateTheme(d))
    ...
    # A FOURTH TIME? WILL THERE BE A FIFTH? (yes! there will be!)
    def themeCreation(self, subs=True):
        if subs:
        ...
        for java in javalist:
            if not os.path.isfile("./Internals/javas/" + java + midway + expected):
                ...
                jpath = os.path.abspath(
                    "./Internals/javas/" + java + midway + expected)
            self.javabutton = QtWidgets.QToolButton(text=java)
            self.javabutton.clicked.connect(
                lambda _=False, d=java: self.updateJavas(jpath))
...
def filefinders():
    ...
    if not os.path.exists("./Internals/themes"):
        # will probably move this to launcher.py
        os.mkdir("./Internals/themes")
    if not os.path.exists("./Internals/javas"):  # eventually
        # but since Javier.py is the current launcher
        os.mkdir("./Internals/javas")
