...
def dlJava(ver):
    operate = platform.system().lower()
    operate = "mac" if operate == "darwin" else operate
    ...
    java = requests.get(
        url=f"https://api.adoptium.net/v3/binary/latest/{ver}/ga/{operate}/{arch}/jre/hotspot/normal/eclipse", headers=header, stream=True)
    ...
    try:
        if operate == "windows":
            ...
            # gotta love that all linux distros + mac will eat a tar.gz just fine
            ftr = file.getnames()[0]
...
class ServerThread(QThread):
    ...
    def run(self):
        ...
        if os.name == "nt":
            ...
            if self.gui == "nogui":
                cmd = "start cmd /k " + cmd
            # i wanted it to use the CMD. not the jar gui
            subprocess.run(cmd, shell=True, cwd=universe)
        ...
        # xterm -e    # MacOS hates this.  will have to determine a workaround for Mac, eventually.
        else:
            cmd = (f"{java} -Xmx{self.RAM}G -Xms256M {jra} -jar {jar} {self.gui}")
            if self.gui == "nogui":
